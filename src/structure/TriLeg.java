		...
		for (int i = 0; i < 3; i++) {// Three corner of legset
			...
			if (i == 0) {
				...
				leg[1].location = chamfer.end;
				 ...
				 Vector4f evenLegLocation = new
				 Vector4f(leg[(i-1)*2].location,0);
				 Vector4f oddLegLocation = new
				 Vector4f(leg[(i-1)*2+1].location,0);
...
//				leg[i * 2].location.add(leg[0].location);
//				leg[i * 2 + 1].location.add(leg[1].location);
				leg[i * 2].rotation.y = (float) Math.toRadians(120 * i);
				leg[i * 2 + 1].rotation.y = (float) Math.toRadians(120 * i);
